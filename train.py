...
def train(
    ...
    for idx, batch in enumerate(data_loader):
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"[WARN] Skipping batch {idx}: loss={loss.item()}")
            continue
...
def parse_args():
    ...
    parser.add_argument("--norm_stats_path", type=str, default="./norm_stats.json", help="Path to normalization stats file")
...
norm_stats = InnerTrackSelectionDataset.fit_normalization_stats(train_set)
track_mean, track_std = norm_stats['track']
seg_mean, seg_std = norm_stats['seg']
hit_mean, hit_std = norm_stats['hit']
print(f"[INFO] Track feature mean: {track_mean}, std: {track_std}")
print(f"[INFO] Seg feature mean: {seg_mean}, std: {seg_std}")
print(f"[INFO] Hit feature mean: {hit_mean}, std: {hit_std}")

norm_stats_path = Path(args.norm_stats_path)
norm_stats_path.parent.mkdir(parents=True, exist_ok=True)
with open(norm_stats_path, "w") as f:
    json.dump(norm_stats, f, indent=2)
print(f"[INFO] Normalization statistics saved to {norm_stats_path}")

print("[INFO] Computing pos_weight from training dataset...")
num_pos, num_neg = 0, 0
for td in train_set:
    target = td['target']
    num_pos += torch.sum(target == 1).item()
    num_neg += torch.sum(target == 0).item()

pos_weight_value = num_neg / max(num_pos, 1)  # divide-by-zero safeguard
pos_weight = torch.tensor(pos_weight_value, dtype=torch.float32)
print(f"[INFO] Computed pos_weight: {pos_weight_value:.4f} (N_pos={num_pos}, N_neg={num_neg})")
...
preprocessor = TensorDictSequential([
    TensorDictModule(
        module=Normalize(mean=track_mean, std=track_std),
        in_keys=['track'],
        out_keys=['track'],
    ),
    TensorDictModule(
        module=Normalize(mean=seg_mean, std=seg_std),
        in_keys=['seg'],
        out_keys=['seg'],
    ),
    TensorDictModule(
        module=Normalize(mean=hit_mean, std=hit_std),
        in_keys=['hit'],
        out_keys=['hit'],
    ),
])
...
logger['pos_weight'] = pos_weight_value
