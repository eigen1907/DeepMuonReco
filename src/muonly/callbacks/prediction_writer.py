...
class PredictionWriter(BasePredictionWriter):
    ...
    def setup(self, trainer, pl_module, stage: str) -> None:
        if stage == "predict":
            if self.output_file_path is None:
                _logger.info(
                    "output_file_path is not set. Using default path from Hydra runtime config."
                )

                log_dir_path = Path(HydraConfig.get().runtime.output_dir)
                if not log_dir_path.exists():
                    raise FileNotFoundError(
                        f"hydra:runtime.output_dir does not exist: {log_dir_path}"
                    )
    ...
    def extend_vlen(self, dataset: Dataset | None, chunk: Tensor, mask: Tensor) -> None:
        ...
        chunk_np = np.array(
            chunk_np, dtype=object
        )  # dtype=object for variable-length arrays
